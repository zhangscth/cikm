...
x = tf.placeholder('float',[None,14*4*101*101])
...
with tf.Session() as sess:
    ...
    for e in range(epoch):
        ...
        # pdb.set_trace()
        [ loss_, y_hat, loss_1_] = sess.run([loss, y_, loss_1],
                                              feed_dict={x: x_batch, y: y_batch, lr: learning_rate})
        print "epoch:", e, " =================== validate loss", loss_
    ...
    with open("/home/zsc/下载/data_new (3)/CIKM2017_testA/testA.txt", 'r') as f:
        ...
        while (1):
            ...
            data = np.reshape(X_train, [-1, 15,4, 101, 101])
            x_train_sub=[]
            ...
            x_train_feature = np.reshape(data, [-1, 14*4 * 101* 101])
